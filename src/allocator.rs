...
const _: () = assert!(HEADER_SIZE.count_ones() == 1); // Size of Header should be power of 2
...
impl Header {
    fn can_provide(&self, size: usize, align: usize) -> bool {
        ...
        self.size >= (HEADER_SIZE + size) + (HEADER_SIZE + align)
    }
    ...
    fn provide(&mut self, size: usize, align: usize) -> Option<*mut u8> {
        ...
        //                                        allocated region
        //
        // header_for_allocated.end_addr() < self.end_addr() の場合、
        // padding用のヘッダーを作成する
        //
