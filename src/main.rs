...
fn locate_graphic_protocol<'a>(
    ...
    let locate_protocol = efi_system_table.boot_services.locate_protocol;
...
fn lookup_font(c: char) -> Option<[[char; 8]; 16]> {
    ...
    while let Some(line) = fi.next() {
        let Some(line) = line.strip_prefix("0x") else {
            continue;
        };
        let Ok(idx) = u8::from_str_radix(line, 16) else {
            continue;
        };
        if idx != c {
            continue;
        ...
        for (y, line) in fi.clone().take(16).enumerate() {
            for (x, c) in line.chars().enumerate() {
                let Some(e) = font[y].get_mut(x) else {
                    continue;
                };
                *e = c;
            }
...
impl Write for VramTextWriter<'_> {
...
fn exit_from_efi_boot_services(
    ...
    loop {
        ...
        let exit_boot_services = efi_system_table.boot_services.exit_boot_services;
        let status = exit_boot_services(image_handle, memory_map.map_key);
        if status == EfiStatus::Success {
            break;
        }
    }
